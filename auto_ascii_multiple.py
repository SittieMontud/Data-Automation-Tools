...
def parsing(stringPathEntry, stringContEntry, outputtextASCII):
	#Configure time
	...
	#Create workbook
	workbookPath = stringPathEntry + "/Parsed_" + timestamp + ".xlsx"
	...
	contLoop = 1
	stopTestname = 0
	for controllernum in range(1, maxCont + 1): 
		#Open the text file
		textpath = stringPathEntry + '/C' + str(controllernum) + '.txt'
		outputtextASCII.insert(tkinter.END, "-Opening file %s " % textpath)
		try:
			datalog = open(textpath, "r")
		except FileNotFoundError:
			outputtextASCII.insert(tkinter.END,"ERROR! File not found. Please make sure to rename your files in this format: C1.txt, C2.txt....")
			break
		...
		flagEnd = re.findall(r"End Test Cycle Count\n", stringAll)
		...
		for loop in range(len(flagStart)):
			...
			dataTests = []
			startRange = dataList.index(flagStart[loop]) + 4
			endRange = dataList.index(flagEnd[loop])+ endRange + 1
			...
			if stopTestname == 0:
				...
				for test in dataTests:
					...
					if len(strRow.split(' ')) > 15:
						dataRow = helpers.MakeTestName(strRow)
						testNames.append(dataRow[2])
						testLSL[dataRow[2]] = []
						testUSL[dataRow[2]] = []
				#Loop for testlimits
				for test in dataTests:
					...
					if len(strRow.split(' ')) > 15:
						dataRow = helpers.MakeTestName(strRow)
						limitlist = testLSL[dataRow[2]]
						limitsL = float("{:.3f}".format(float(helpers.LowTestLimits(strRow))))
						limitlist.append(limitsL)
						limitlist = testUSL[dataRow[2]]
						limitsU = float("{:.3f}".format(float(helpers.HighTestLimits(strRow))))
			...
			for num in range(0,len(dutList)):
				...
				listVal[dutList[num]].insert(1, controllernum)	#Inserting controller to list
				...
				for test in dataTests:
					strRow = ""
					strRow = helpers.RemoveSpaces(test, strRow)
					if len(strRow.split(' ')) > 15: #Functional Tests not included
						listdummy = strRow.split(' ')
						if (int(dutList[num]) == int(listdummy[1])):
							measvalues = float("{:.3f}".format(float(helpers.StoreValues(strRow))))
